...
public class HDRIToCubemapFaces {
  ...
  private static void generateCubemapFaces(HDRTexture hdrTexture, String outputDirectory)
      throws Exception {
  ...
  private static ByteBuffer extractFace(HDRTexture hdrTexture, Vector3 faceDir, Vector3 upDir) {
    ByteBuffer faceBuffer =
        ByteBuffer.allocateDirect(CUBEMAP_SIZE * CUBEMAP_SIZE * 3 * Float.BYTES);
  ...
  private static void saveFaceAsImage(
      ByteBuffer faceData, int width, int height, String outputPath) {
